...
async function sendimg() {
  ...
  if (!selectedOption) {
    alert("系统未能识别当前墨水屏驱动类型。如未显示相关配置，请在底部点击开启[开发模式]后手动挑选驱动再此点击发送。");
    return;
  }
...
function handleNotify(value, idx) {
  ...
  if (idx == 0) {
    ...
    if (epddriver.selectedIndex === -1) {
      addLog(`未知驱动ID 0x${bytes2hex(data.slice(7, 8))}，已退回至默认选项。`);
      epddriver.selectedIndex = 0;
    }
