...
- **上传**：两处（服务端按 `ls` 去重，重复无害）——① 拉照片**之前**（setup 9.5 步，
  兼顾临时 server 模式：那种模式检测到拉取就关闭，睡前那次会扑空）；② 入睡前
  （`goDeepSleepMinutes()`，覆盖手动唤醒窗口内的事件）。把 `ls > 上传游标` 的增量
  按旧→新分批（每批 ≤6KB）POST 到 `POST /api/device_log/<key>`；收到 200 才推进
  游标（存 NVS）。失败保留重传。
